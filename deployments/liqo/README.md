...
| networking.fabric.config.notrack | object | `{"enabled":false}` | Enable/Disable the creation of the NOTRACK firewallconfiguration for the fabric pod. When enabled, UDP traffic on the geneve port is exempted from connection tracking, which reduces overhead and prevents the conntrack table from being flooded by tunnel traffic. |
