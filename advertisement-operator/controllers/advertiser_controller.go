...
func (r *AdvertiserReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Create(ctx, &providerConfigMap, &client.CreateOptions{}) ; err != nil {
	...
	if err := r.Create(ctx, &remoteKubeConfigMap, &client.CreateOptions{}) ; err != nil {
		log.Error(err, "unable to create configMap")
		// we'll ignore not-found errors, since they can't be fixed by an immediate
		// requeue (we'll need to wait for a new notification), and we can get them
		// on deleted requests.
	...
	args[2] = "--provider-config" ; args[3] = "/home/francesco/vkubelet-cfg.json"
					...
					Containers: []v1.Container{
						{
							...
							Command:    command,
							Args:       args,
							WorkingDir: "/home/francesco/go/src/github.com/",
						},
					},
				},
			},
	...
	if err := r.Create(ctx, &deploy, &client.CreateOptions{}); err != nil{
