...
The class a `ResourceSlice` is handled with is therefore always the one reported in `status.class`, which the provider fills in with the requested class, or with its default one when no class is requested: a custom controller must select the `ResourceSlice`s it is responsible for through `status.class`.
