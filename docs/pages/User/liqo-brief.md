...
## Overview
Liqo is an open source add-on for Kubernetes that allow to seamlessly and securely share resources and services between multiple clusters, enabling to run your pods on another cluster.
...
Kubernetes users will experience the usual environment also after starting Liqo: all administration tasks are the same, performed in the usual way and with the well-known tools. The only difference is that your cluster can become more powerful, as resources and services can be borrowed from the other clusters you peered with.
...
## Terminology
We call **home** cluster the one under your control, while the **foreign** cluster is the one that you peer to, which is usually under the control of some other organizations.
...
This mode is meaningful particularly for single-node clusters (e.g., a laptop running Liqo) when they connect to enterprise infrastructure, which has the right to control all the computing nodes at their full extent.
...
When the tethered peering is established:
  ...
  * Every resource request made by the local cluster is forwarded to the foreign cluster that will perform a proper scheduling. In this way:
      ...
      * The enterprise cluster can achieve a more efficient resource allocation, easily sharing its computational power and application logic with its own employees, which may result in a significant cost reduction.
...
> **NOTE**: Since the tethering require only one unidirectional peering, transition from _Autonomous_ to _Tethered_ mode is allowed only in presence of **at most one peering** in which the local cluster is **offering** resources.
