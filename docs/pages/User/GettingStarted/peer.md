...
To allow other cluster to peer with your cluster(s), it you need to register a set of DNS records that specify the cluster(s) available in your domain, with the different parameters required to establish the connection.
...
* The `A` record assigns an IP address to the DNS name of the Kubernetes API server, in this case `1.2.3.4`.
  Although, for the sake of simplicity, this example configures an `A` record, an indirect `CNAME` record would also work.
