...
Liqo exposes the **gateway server** using an UDP service.

in the majiority of the cases, the issue is related to the missing support for UDP services on a cloud provider or in your on-premise environment.
...
If you get an error in their status, refer to the [Debug gateway-to-gateway communication issues](./faq.md#FAQDebugGatewayToGateway) section.
