...
In the internal network, each node is connected to the pod acting as **gateway** (one for each peering) using a dedicated Geneve tunnel.
...
The encapsulated traffic is taken by the CNI and routed to the Gateway, as formally this traffic looks like a packet generated by the node itself.
...
In general, GENEVE adds, without options, an overhead of 50 bytes (14 bytes Ethernet header, 20 bytes IPv4 header, 8 bytes UDP header, and 8 bytes GENEVE header)
...
This traffic arrives at the gateway via the default interface, directed to a GENEVE socket listening on port 6091. The traffic is decapsulated by Geneve code and reintroduced into the gateway stack.
...
_Note_: Within the Gateway, a route is also configured to direct all traffic arriving from the Wireguard interface and destined for a local pod back to the appropriate GENEVE interface.
...
In Liqo, when the Gateway is created, the local WireGuard interface is set to contact the appropriate endpoint, which in this case is the address of the node hosting the remote Gateway.
...
In fact, all the remapping logic is managed locally by the Liqo IPAM component, which acts as the brain behind CIDR and IP address management within the cluster. It tracks the used IPs and CIDRs inside the cluster, and therefore can simply decide which IP address ranges could be used in the remapping process. Unlike traditional IPAM solutions, which typically only store allocated CIDRs or IPs, Liqo IPAM can dynamically allocate and return an available CIDR or IP address even if the originally requested one is already in use.

All the NAT rules for managing this remapping are handled inside the gateway pods, ensuring seamless communication even in the presence of overlapping CIDRs.

The same remapping logic also applies to the **External CIDR**.
