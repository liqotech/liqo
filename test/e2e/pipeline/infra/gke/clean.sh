...
do
  ...
  if gcloud container clusters describe "${GKE_CLUSTER_ID}" --zone "${GKE_CLUSTER_ZONE}" --project "${GKE_PROJECT_ID}" &> /dev/null; then
    "${GCLOUD}" container clusters delete "${GKE_CLUSTER_ID}" --zone "${GKE_CLUSTER_ZONE}" --project "${GKE_PROJECT_ID}" --quiet &
    PIDS+=($!)
  fi
done
