...
func cleanUp(t *testing.T, localResources map[string]*netv1alpha1.TunnelEndpoint) {
	for _, res := range localResources {
		err := dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").Delete(context.TODO(), res.Name, metav1.DeleteOptions{})
	...
	for clusterID, dynClient := range peeringClustersDynClients {
		_, err := dynClient.Resource(tunGVR).Namespace("default").Get(context.TODO(), localResources[clusterID].Name, metav1.GetOptions{})
...
func TestReplication1(t *testing.T) {
	...
	newTun, err := dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").Create(context.TODO(), tun, metav1.CreateOptions{})
	assert.Nil(t, err, "error should be nil")

	time.Sleep(2 * time.Second)
	// check that the resource does not exist on the remote clusters
	for _, dynClient := range peeringClustersDynClients {
		_, err := dynClient.Resource(tunGVR).Namespace("default").Get(context.TODO(), tun.GetName(), metav1.GetOptions{})
		assert.True(t, apierrors.IsNotFound(err), "error should be not found")
	}
	// delete resources
	err = dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").Delete(context.TODO(), newTun.GetName(), metav1.DeleteOptions{})
	assert.Nil(t, err, "error should be nil")
...
func TestReplication2(t *testing.T) {
	...
	for clusterID, dynClient := range peeringClustersDynClients {
		typedTun := &netv1alpha1.TunnelEndpoint{}
		remTun, err := dynClient.Resource(tunGVR).Namespace("default").Get(context.TODO(), localResources[clusterID].Name, metav1.GetOptions{})
		assert.Nil(t, err, "error should be nil")
...
func TestReplication4(t *testing.T) {
	...
	for clusterID, tun := range localResources {
		...
		currentTun, err := peeringClustersDynClients[clusterID].Resource(tunGVR).Namespace("default").Get(context.TODO(), tun.Name, metav1.GetOptions{})
		assert.Nil(t, err, "error should be nil")
		err = unstructured.SetNestedMap(currentTun.Object, status, "status")
		assert.Nil(t, err, "error should be nil")
		_, err = peeringClustersDynClients[clusterID].Resource(tunGVR).Namespace("default").UpdateStatus(context.TODO(), currentTun, metav1.UpdateOptions{})
		assert.Nil(t, err, "error should be nil")
		time.Sleep(10 * time.Second)
	}
	...
	for _, tun := range localResources {
		remTun, err := dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").Get(context.TODO(), tun.GetName(), metav1.GetOptions{})
		assert.Nil(t, err, "error should be nil")
		typedTun := &netv1alpha1.TunnelEndpoint{}
		err = runtime.DefaultUnstructuredConverter.FromUnstructured(remTun.Object, typedTun)
		assert.Nil(t, err, "error should be nil")
...
func TestReplication3(t *testing.T) {
	...
	for _, tun := range localResources {
		...
		currentTun, err := dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").Get(context.TODO(), tun.Name, metav1.GetOptions{})
		assert.Nil(t, err, "error should be nil")
		err = unstructured.SetNestedMap(currentTun.Object, status, "status")
		assert.Nil(t, err, "error should be nil")
		_, err = dOperator.LocalDynClient.Resource(tunGVR).Namespace("default").UpdateStatus(context.TODO(), currentTun, metav1.UpdateOptions{})
		assert.Nil(t, err, "error should be nil")
		time.Sleep(10 * time.Second)
	}
