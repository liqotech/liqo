...
func (n *node) hasExclusiveDescendants() bool {
	if n.left != nil && (n.left.isExclusive() || n.left.hasExclusiveDescendants()) {
		return true
	}
	if n.right != nil && (n.right.isExclusive() || n.right.hasExclusiveDescendants()) {
		return true
	}
	return false
}
...
// exclusive=false: ref-counted overlapping (refCount>0)
// exclusive=true: sole ownership (refCount=-1)
// Exclusive nodes block all acquisitions on the same prefix, children, and ancestors.
func allocateNetworkWithPrefix(prefix netip.Prefix, node *node, exclusive bool) *netip.Prefix {
	...
	if node.prefix.Addr().Compare(prefix.Addr()) == 0 && node.prefix.Bits() == prefix.Bits() {
		if exclusive {
			...
			if node.isExclusive() || node.hasExclusiveDescendants() {
				return nil
			}
			node.refCount++
		}
