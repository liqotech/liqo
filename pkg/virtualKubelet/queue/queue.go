...
// Package queue abstracts a workqueue and a set of associated workers.
package queue
...
// Queue implements a wrapper around workqueue.
type Queue struct {
	lock      sync.Mutex
	running   bool
	name      string
...
//
// It blocks until context is canceled, and all of the workers exit.
...
func (q *Queue) handleQueueItem(ctx context.Context) bool {
	...
	key := obj.(string)
	klog.V(5).Infof("Got Queue object %q", key)

	err := q.handleQueueItemObject(ctx, key)
	if err != nil {
		// We've actually hit an error, so we set the span's status based on the error.
		klog.Errorf("Error processing Queue item %q: %s", key, err.Error())
		return true
	}
	klog.V(5).Infof("Processed Queue object %q", key)
	return true
}
...
func (q *Queue) handleQueueItemObject(ctx context.Context, key string) error {
	...
	defer func(start time.Time) {
		klog.V(5).Infof("Queue item object %q handled in %v", key, time.Since(start))
	}(time.Now())
