		...
		t.Run(env, func(t *testing.T) {
			...
			t.Run("createStartDeleteScenarioWithDeletionRandomError", func(t *testing.T) {
				...
				assert.NilError(t, wireUpSystem(ctx, mp, func(ctx context.Context, s *system) {
					...
					pods, err := s.client.CoreV1().Pods(testNamespace).List(context.TODO(), metav1.ListOptions{})
...
func testTerminalStatePodScenario(ctx context.Context, t *testing.T, s *system, state corev1.PodPhase) {
	...
	// Create the Pod
	_, e := s.client.CoreV1().Pods(testNamespace).Create(context.TODO(), p1, metav1.CreateOptions{})
	assert.NilError(t, e)
	...
	p2, err := s.client.CoreV1().Pods(testNamespace).Get(context.TODO(), p1.Name, metav1.GetOptions{})
	assert.NilError(t, err)
...
func testDanglingPodScenarioWithDeletionTimestamp(ctx context.Context, t *testing.T, s *system, m testingProvider) {
	...
	watcher, err := s.client.CoreV1().Pods(testNamespace).Watch(context.TODO(), listOptions)
	assert.NilError(t, err)
	defer watcher.Stop()
	...
	_, e := s.client.CoreV1().Pods(testNamespace).Create(context.TODO(), podCopyWithDeletionTimestamp, metav1.CreateOptions{})
	assert.NilError(t, e)
...
func testCreateStartDeleteScenario(ctx context.Context, t *testing.T, s *system, waitFunction func(ctx context.Context, watch watch.Interface) error, waitForPermanentDeletion bool) {
	...
	// Create the Pod
	_, e := s.client.CoreV1().Pods(testNamespace).Create(context.TODO(), p, metav1.CreateOptions{})
	assert.NilError(t, e)
	...
	watcher2, err := s.client.CoreV1().Pods(testNamespace).Watch(context.TODO(), listOptions)
	assert.NilError(t, err)
	defer watcher2.Stop()
	...
	// 1. Get the pod
	currentPod, err := s.client.CoreV1().Pods(testNamespace).Get(context.TODO(), p.Name, metav1.GetOptions{})
	assert.NilError(t, err)
	...
	_, err = s.client.CoreV1().Pods(testNamespace).Update(context.TODO(), currentPod, metav1.UpdateOptions{})
	assert.NilError(t, err)
...
func testUpdatePodWhileRunningScenario(ctx context.Context, t *testing.T, s *system, m testingProvider) {
	...
	_, err = s.client.CoreV1().Pods(p.Namespace).Update(context.TODO(), p, metav1.UpdateOptions{})
	assert.NilError(t, err)
...
func benchmarkCreatePods(ctx context.Context, b *testing.B, s *system) {
	...
	for i := 0; i < b.N; i++ {
		...
		_, err := s.client.CoreV1().Pods(pod.Namespace).Create(context.TODO(), pod, metav1.CreateOptions{})
