	...
	Describe("Testing the NetworkChecker", func() {
		Context("Collecting and retrieving the data", func() {
			...
			It("should preserve multiple remote and remapped CIDRs", func() {
				...
				expectedExternalCIDRs := []string{"10.0.2.0/24", "10.0.3.0/24"}
				expectedRemappedExternal := []string{"11.0.2.0/24", "11.0.3.0/24"}
			...
			Entry("Healthy module with multiple CIDRs", Network{
				...
				CIDRs: CIDRInfo{
					Remote: networkingv1beta1.ClusterConfigCIDR{
						...
						External: cidrutils.FromStrings([]string{"10.0.2.0/24", "10.0.3.0/24"}),
					},
					Remapped: &networkingv1beta1.ClusterConfigCIDR{
						Pod:      cidrutils.FromStrings([]string{"11.0.0.0/24", "11.0.1.0/24"}),
						External: cidrutils.FromStrings([]string{"11.0.2.0/24", "11.0.3.0/24"}),
					},
				},
				Gateway: GatewayInfo{
