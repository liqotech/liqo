...
var _ = Describe("Pods", func() {
	...
	Describe("PodState", func() {
		...
		Describe("creating a new podState", func() {
			It("should hold the passed parameters during the creation", func() {
				ps := newPodState(deploymentType)
				Expect(ps.deploymentType).To(Equal(deploymentType))
			})
		})
		...
		Describe("handling images field", func() {
			Context("getting images", func() {
				It("should return a slice of length 1", func() {
					im := podS.getImages()
					Expect(len(im)).To(BeNumerically("==", 1))
		...
		Describe("handling errorsPerPod field", func() {
			var (
				podName = "pod1"
				err     = errors.New("new pod error")
			)

			Context("adding error per pod", func() {
				When("no errors per pod exists", func() {
					It("should add the error for the given pod", func() {
						podS.errors = nil
						podS.addErrorPerPod(podName, err)
						Expect(len(podS.errors[podName].errors)).To(BeNumerically("==", 1))
						Expect(podS.errors[podName].errors[0]).To(MatchError(err))
					})
				})

				When("errors per pod exists", func() {
					It("should append the new error for the given pod", func() {
						podS.addErrorPerPod(podName, err)
						Expect(len(podS.errors[podName].errors)).To(BeNumerically("==", 2))
		...
		Describe("formatting the podState", func() {
			...
			Context("collecting deployment apps", func() {
				When("fails to get the deployment", func() {
					It("should add an error to the collectionErrors", func() {
						...
						Expect(len(podC.collectionErrors)).To(BeNumerically("==", 2))
					})
				})
				...
				When("deployment and pod exist", func() {
					It("should not add errors", func() {
						...
						Expect(podC.Collect(ctx)).To(BeNil())
						Expect(podC.errors).To(BeTrue())
						Expect(len(podC.collectionErrors)).To(BeNumerically("==", 1))
					})
				})
			})
