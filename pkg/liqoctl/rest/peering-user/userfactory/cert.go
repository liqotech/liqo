...
func GeneratePeerUser(ctx context.Context, clusterID liqov1beta1.ClusterID, tenantNsName string, opts *factory.Factory) (string, error) {
	if exists, err := IsExistingPeerUser(ctx, opts.CRClient, clusterID); err != nil {
		...
		return "", fmt.Errorf("a user to peer from cluster with ID %q already exists. Please delete if first before creting a new one."+
	...
	kubeconfig, err := generateKubeconfig(string(clusterID), ca, cert, privatePEM, apiAddr)
	if err != nil {
...
func generateKubeconfig(clusterID string, ca, cert, private []byte, apiAddr string) (string, error) {
	...
	kubeconfig.Clusters[clusterID] = cluster

	// Set up the user
	user := api.NewAuthInfo()
	user.ClientCertificateData = cert
	user.ClientKeyData = private
	kubeconfig.AuthInfos[userName] = user

	// The up the clusterContext
	clusterContext := api.NewContext()
	clusterContext.Cluster = clusterID
	clusterContext.AuthInfo = userName
	kubeconfig.Contexts[clusterID] = clusterContext
	kubeconfig.CurrentContext = clusterID

	// Convert the kubeconfig to YAML
