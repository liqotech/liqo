...
func (o *Options) Get(ctx context.Context, options *rest.GetOptions) *cobra.Command {
	...
	cmd.Flags().BoolVar(&o.onlyClusterName, "cluster-name", false, "Print only the cluster name")
...
func (o *Options) handleGet(ctx context.Context) error {
	localClusterIdentity, err := liqoutils.GetClusterIdentityWithControllerClient(ctx, o.getOptions.CRClient, o.getOptions.LiqoNamespace)
	if err != nil {
		return err
	}
	...
	case o.onlyClusterID:
		fmt.Println(clusterConfig.ClusterIdentity.ClusterID)
		return nil
	case o.onlyClusterName:
		fmt.Println(clusterConfig.ClusterIdentity.ClusterName)
		return nil
	}
