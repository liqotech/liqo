...
// BindingTarget represents a single entity (an InternalNode or a gateway pod) that needs a
// FirewallConfigurationBinding. It carries a full GroupVersionKind so that the garbage collector
// can resolve and observe the target generically.
type BindingTarget struct {
	// APIVersion of the target (e.g. "v1" for a Pod or "networking.liqo.io/v1beta1" for an InternalNode).
	APIVersion string
	// Kind of the target (e.g. "Pod" or "InternalNode").
	Kind string
	// Name is the node name (for fabric) or gateway pod name (for gateway).
	Name string
	// Namespace of the target. Empty for cluster-scoped targets (e.g. an InternalNode).
	Namespace string
}
...
func (b *BindingCreatorBase) ReconcileBindings(ctx context.Context,
	...
	for i := range targets {
		t := &targets[i]
		bindingName := BindingResourceName(fwcfg.Name, t.Name)
		if err := b.ensureBinding(ctx, fwcfg, bindingName, t); err != nil {
...
// ensureBinding creates or updates a FirewallConfigurationBinding for the given target.
func (b *BindingCreatorBase) ensureBinding(ctx context.Context,
	fwcfg *networkingv1beta1.FirewallConfiguration, bindingName string, target *BindingTarget) error {
	...
	op, err := controllerutil.CreateOrUpdate(ctx, b.Client, binding, func() error {
		...
		binding.Spec.TargetRef = networkingv1beta1.TargetReference{
