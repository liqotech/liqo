...
var _ = Describe("updateStatus", func() {
	...
	It("sets ConditionFalse on reconcile error and returns nil when status update succeeds", func() {
		a := newBinding("a", nil)
		r, _ := newBindingReconciler(a)
		recErr := errors.New("boom")

		Expect(r.updateStatus(ctx, a, recErr)).To(Succeed())
...
var _ = Describe("forgeTargetRefPredicate", func() {
	const (
		testAPIVersion = "v1"
		testKind       = "Node"
		testName       = "my-node"
	...
	makeObj := func(name, namespace string) client.Object {
		return &networkingv1beta1.FirewallConfigurationBinding{
			...
			Spec: networkingv1beta1.FirewallConfigurationBindingSpec{
				TargetRef: networkingv1beta1.TargetReference{
	...
	testTarget := networkingv1beta1.TargetReference{
		APIVersion: testAPIVersion,
		Kind:       testKind,
		Name:       testName,
	...
	It("matches objects whose spec.targetRef equals the configured value", func() {
		p := ForgeTargetRefPredicate(testTarget)

		Expect(p.Create(event.CreateEvent{Object: makeObj(testName, testNamespace)})).To(BeTrue())
		Expect(p.Update(event.UpdateEvent{ObjectNew: makeObj(testName, testNamespace)})).To(BeTrue())
		Expect(p.Delete(event.DeleteEvent{Object: makeObj(testName, testNamespace)})).To(BeTrue())
	})

	It("does not match objects with a different spec.targetRef", func() {
		p := ForgeTargetRefPredicate(testTarget)

		Expect(p.Create(event.CreateEvent{Object: makeObj("other-node", "")})).To(BeFalse())
		Expect(p.Create(event.CreateEvent{Object: makeObj(testName, "other-ns")})).To(BeFalse())
