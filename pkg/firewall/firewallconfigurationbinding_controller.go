...
// apiVersion, kind, name and namespace identify the entity running this controller
// (e.g. an InternalNode or a gateway pod); only FirewallConfigurationBinding resources whose
// spec.targetRef matches are reconciled.
func (r *FirewallConfigurationBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager,
	apiVersion, kind, name, namespace string, enableNftMonitor bool, reconcileTimeout time.Duration) error {
	klog.Infof("Starting FirewallConfigurationBinding controller for target %s/%s %s/%s", apiVersion, kind, namespace, name)
	...
	return ctrl.NewControllerManagedBy(mgr).Named(consts.CtrlFirewallConfigurationBinding).
		For(&networkingv1beta1.FirewallConfigurationBinding{}, builder.WithPredicates(ForgeTargetRefPredicate(apiVersion, kind, name, namespace))).
		Watches(&networkingv1beta1.FirewallConfiguration{},
			NewFirewallConfigurationBindingByConfigurationHandler(r.Client, apiVersion, kind, name, namespace)).
		WatchesRawSource(NewFirewallBindingWatchSource(src, NewFirewallBindingWatchEventHandler(r.Client, apiVersion, kind, name, namespace))).
...
// whose spec.targetRef equals the given apiVersion, kind, name and namespace.
func ForgeTargetRefPredicate(apiVersion, kind, name, namespace string) predicate.Predicate {
	return predicate.NewPredicateFuncs(func(obj client.Object) bool {
		binding, ok := obj.(*networkingv1beta1.FirewallConfigurationBinding)
		if !ok {
			return false
		}
		return MatchesTargetRef(&binding.Spec.TargetRef, apiVersion, kind, name, namespace)
	})
}

// MatchesTargetRef returns true if the given TargetReference matches the provided
// apiVersion, kind, name and namespace.
func MatchesTargetRef(ref *networkingv1beta1.TargetReference, apiVersion, kind, name, namespace string) bool {
	return ref.APIVersion == apiVersion &&
		ref.Kind == kind &&
		ref.Name == name &&
