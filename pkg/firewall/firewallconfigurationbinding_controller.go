...
func NewFirewallConfigurationBindingReconciler(cl client.Client, s *runtime.Scheme,
...
func (r *FirewallConfigurationBindingReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			...
			return ctrl.Result{}, fmt.Errorf("referenced firewallconfiguration %q not found",
	...
	if err = r.NftConnection.Flush(); err != nil {
		return ctrl.Result{}, fmt.Errorf("flushing nftables connection after table cleanup: %w", err)
	}
	...
	if err = r.NftConnection.Flush(); err != nil {
		return ctrl.Result{}, fmt.Errorf("flushing nftables connection after table creation: %w", err)
	}
	...
	// That function iterates on every conntrack flow, that's why we run it only when
...
// only FirewallConfigurationBinding resources whose spec.targetRef matches are reconciled.
func (r *FirewallConfigurationBindingReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager,
	target networkingv1beta1.TargetReference, enableNftMonitor bool, reconcileTimeout time.Duration) error {
	klog.Infof("Starting FirewallConfigurationBinding controller for target %s", target.String())
	...
	return ctrl.NewControllerManagedBy(mgr).Named(consts.CtrlFirewallConfigurationBinding).
		For(&networkingv1beta1.FirewallConfigurationBinding{}, builder.WithPredicates(ForgeTargetRefPredicate(target))).
		Watches(&networkingv1beta1.FirewallConfiguration{},
			NewFwCfgToFwCfgBindingEnqueuer(r.Client, target)).
		WatchesRawSource(NewFirewallBindingWatchSource(src, NewFwCfgBindingEnqueuer(r.Client, target))).
...
func (r *FirewallConfigurationBindingReconciler) updateStatus(ctx context.Context,
	...
	conditionReason := string(networkingv1beta1.FirewallConfigurationBindingConditionReasonApplySucceeded)
	conditionMessage := ""
	if reconcileErr != nil {
		conditionReason = string(networkingv1beta1.FirewallConfigurationBindingConditionReasonApplyFailed)
...
// ForgeTargetRefPredicate returns a predicate that matches FirewallConfigurationBinding resources
// whose spec.targetRef equals the given target.
func ForgeTargetRefPredicate(target networkingv1beta1.TargetReference) predicate.Predicate {
	return predicate.NewPredicateFuncs(func(obj client.Object) bool {
		binding, ok := obj.(*networkingv1beta1.FirewallConfigurationBinding)
		if !ok {
			return false
		}
		return MatchesTargetRef(&binding.Spec.TargetRef, target)
	})
}

// MatchesTargetRef returns true if the given TargetReference matches the provided target.
func MatchesTargetRef(ref *networkingv1beta1.TargetReference, target networkingv1beta1.TargetReference) bool {
