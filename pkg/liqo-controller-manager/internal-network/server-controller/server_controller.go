...
func (r *ServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if err = r.ensureInternalFabric(ctx, gwServer, configuration); err != nil {
	...
	if _, err := controllerutil.CreateOrUpdate(ctx, r.Client, internalFabric, func() error {
		...
		if gwServer.Status.InternalEndpoint != nil && gwServer.Status.InternalEndpoint.IP != nil {
			internalFabric.Spec.Endpoint = &networkingv1alpha1.InternalEndpoint{
				IP:   *gwServer.Status.InternalEndpoint.IP,
				Port: 1234, // TODO: set the geneve port
			}

			if gwServer.Status.InternalEndpoint.Node != nil {
				internalFabric.Spec.NodeName = *gwServer.Status.InternalEndpoint.Node
			}
		}
		...
		gwIP, err := gwIpam.Allocate(fmt.Sprintf("%s/%s", gwServer.Namespace, gwServer.Name))
		if err != nil {
			return err
		}
