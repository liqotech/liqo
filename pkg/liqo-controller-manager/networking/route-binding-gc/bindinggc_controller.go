...
// RouteConfigurationBinding is still linked to a live target entity.
const DefaultBindingGCPeriod = 10 * time.Minute

// BindingGCReconciler garbage-collects RouteConfigurationBinding resources that are no
// longer linked to their referenced target entity.
//
// A RouteConfigurationBinding is created by the fabric or gateway binding creator
// controllers and carries a Spec.TargetRef that identifies the entity (e.g. a Pod or a
// Node) responsible for applying the referenced RouteConfiguration and for removing
...
// RouteConfiguration) stuck forever.
//
...
// removes the controller finalizer so that it (and any owning RouteConfiguration) can be
// collected.
...
// cluster-role
// +kubebuilder:rbac:groups=networking.liqo.io,resources=routeconfigurationbindings,verbs=get;list;watch;update;patch;delete
...
// Reconcile checks whether the given RouteConfigurationBinding is still linked to a live
...
func (r *BindingGCReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, binding); err != nil {
		...
		return ctrl.Result{}, fmt.Errorf("getting routeconfigurationbinding %s: %w", req.String(), err)
	}

	linked, err := r.isTargetLinked(ctx, binding.Spec.TargetRef)
	if err != nil {
		return ctrl.Result{}, fmt.Errorf("checking target link for routeconfigurationbinding %s: %w", req.String(), err)
	}
	...
	klog.Infof("RouteConfigurationBinding %s is not linked to target %s: garbage collecting",
...
func (r *BindingGCReconciler) garbageCollect(ctx context.Context, binding *networkingv1beta1.RouteConfigurationBinding) error {
	...
	if binding.DeletionTimestamp.IsZero() {
		if err := r.Delete(ctx, binding); err != nil {
			...
			return fmt.Errorf("deleting routeconfigurationbinding %s/%s: %w", binding.Namespace, binding.Name, err)
		}
		klog.Infof("Requested deletion of orphaned RouteConfigurationBinding %s/%s", binding.Namespace, binding.Name)
	}
	...
	if ctrlutil.ContainsFinalizer(binding, route.RouteConfigurationBindingControllerFinalizer) {
		original := binding.DeepCopy()
		ctrlutil.RemoveFinalizer(binding, route.RouteConfigurationBindingControllerFinalizer)
		if err := r.Patch(ctx, binding, client.MergeFrom(original)); err != nil {
			if apierrors.IsNotFound(err) {
				return nil
			}
			return fmt.Errorf("removing finalizer from routeconfigurationbinding %s/%s: %w", binding.Namespace, binding.Name, err)
		}
		klog.Infof("Forcefully removed finalizer from orphaned RouteConfigurationBinding %s/%s", binding.Namespace, binding.Name)
	}
...
// namespace empty.
func (r *BindingGCReconciler) isTargetLinked(ctx context.Context, ref networkingv1beta1.RouteBindingTargetReference) (bool, error) {
...
func newUnstructuredForTargetRef(ref networkingv1beta1.RouteBindingTargetReference) (*unstructured.Unstructured, error) {
...
// or a gateway Pod triggers prompt cleanup of the related RouteConfigurationBindings.
func (r *BindingGCReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).Named(consts.CtrlRouteConfigurationBindingGC).
		For(&networkingv1beta1.RouteConfigurationBinding{}).
...
// bindingEnqueuerForTarget maps a deleted Node or Pod to the RouteConfigurationBindings
...
func (r *BindingGCReconciler) bindingEnqueuerForTarget(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.List(ctx, bindingList); err != nil {
		klog.Errorf("Failed to list RouteConfigurationBindings for target %s/%s: %v",
