...
func CreateOrUpdateNatMappingCIDR(ctx context.Context, cl client.Client, opts *Options,
	...
	klog.V(4).Infof("Creating firewall configuration %q for %q", fwcfg.Name, cidrtype)
	...
	if err != nil {
		return fmt.Errorf("enforcing firewall configuration %q for %q: %w", fwcfg.Name, cidrtype, err)
	}

	if op != controllerutil.OperationResultNone {
		klog.Infof("Enforced firewall configuration %q for %q: %s", fwcfg.Name, cidrtype, op)
	}
