...
var _ = Describe("ResourceSlice class resolution", func() {
	...
	Describe("The resolveClass function", func() {
		...
		When("no default class is configured on the provider, and the consumer did not request any", func() {
			BeforeEach(func() {
			...
			It("should report the built-in default class, preserving the previous behavior", func() {
				Expect(resourceSlice.Status.Class).To(BeEquivalentTo(authv1beta1.ResourceSliceClassDefault))
			})
		})
		...
		When("a default class is configured, and the consumer did not request any", func() {
			...
			It("should report the configured class, leaving the spec untouched", func() {
		...
		When("the consumer requested an explicit class", func() {
			...
			It("should report the requested class", func() {
		...
		When("the resources have already been handled, before any class was resolved", func() {
			BeforeEach(func() {
				reconciler = newReconciler(providerClass)
				resourceSlice = handled(newResourceSlice(authv1beta1.ResourceSliceClassUnknown, authv1beta1.ResourceSliceClassUnknown))
			})

			It("should report the built-in default class, which is the one that handled them", func() {
				Expect(resourceSlice.Status.Class).To(BeEquivalentTo(authv1beta1.ResourceSliceClassDefault))
			})
		})
	})
	...
	Describe("The isInResourceClasses function", func() {
		...
		When("the class is resolved to a custom one", func() {
			BeforeEach(func() {
				reconciler = newReconciler(providerClass)
				resourceSlice = newResourceSlice(authv1beta1.ResourceSliceClassUnknown, authv1beta1.ResourceSliceClassUnknown)
			})
		...
		When("the class is resolved to the built-in default one", func() {
			BeforeEach(func() {
		...
		When("the consumer requested an explicit custom class", func() {
			BeforeEach(func() {
				reconciler = newReconciler(authv1beta1.ResourceSliceClassUnknown)
				resourceSlice = newResourceSlice(customClass, authv1beta1.ResourceSliceClassUnknown)
			})

			It("should leave the ResourceSlice to the controller of that class", func() {
				Expect(claimed).To(BeFalse())
			})
		})
	})
