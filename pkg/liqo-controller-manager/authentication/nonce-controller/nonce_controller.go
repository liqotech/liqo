...
// NonceReconciler manage Nonces lifecycle.
...
func NewNonceReconciler(cl client.Client, s *runtime.Scheme,
	namespaceManager tenantnamespace.Manager,
	recorder record.EventRecorder) *NonceReconciler {
...
// cluster-role
// +kubebuilder:rbac:groups=,resources=secrets,verbs=get;list;watch;delete;create;update;patch

// Reconcile manage Nonce lifecycle.
func (r *NonceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	if _, ok := secret.Data[consts.NonceKey]; ok {
		klog.V(4).Infof("Nonce already found in secret %q", req.NamespacedName)
		return ctrl.Result{}, nil
	}

	clusterID, ok := secret.Labels[discovery.ClusterIDLabel]
	if !ok {
	...
	secret.StringData[consts.NonceKey] = nonce
...
func (r *NonceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	req1, err := labels.NewRequirement(consts.NonceLabelKey, selection.Exists, nil)
	if err != nil {
		return err
	}
