...
const (
	...
	// PingBindIPFlag is the name of the flag used to set the ping bind IP.
...
func InitFlags(flagset *pflag.FlagSet, options *Options) {
	...
	flagset.IntVar(&options.PingPort, PingPortFlag.String(), 12345,
		"ping-port is the port used for the ping check")
	flagset.UintVar(&options.PingBufferSize, PingBufferSizeFlag.String(), 1024,
		"ping-buffer-size is the size of the buffer used for the ping check")
	flagset.UintVar(&options.PingLossThreshold, PingLossThresholdFlag.String(), 5,
		"ping-loss-threshold is the number of lost packets after which the connection check is considered as failed.")
	flagset.DurationVar(&options.PingInterval, PingIntervalFlag.String(), 2*time.Second,
	...
	flagset.DurationVar(&options.PingUpdateStatusInterval, PingUpdateStatusIntervalFlag.String(), 10*time.Second,
		"the interval used to send the update to the K8S API server with the update function")
	flagset.StringVar(&options.PingBindIP, PingBindIPFlag.String(), "",
		"ping-bind-ip is the IP address to bind the UDP socket to. Defaults to 0.0.0.0 if empty.")
}
