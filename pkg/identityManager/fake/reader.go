...
type IdentityReader struct {
	configs     map[liqov1alpha1.ClusterID]*rest.Config
	namespaces  map[liqov1alpha1.ClusterID]string
	secretNames map[liqov1alpha1.ClusterID]string
}

// NewIdentityReader creates a new identityReader instance.
func NewIdentityReader() *IdentityReader {
	return &IdentityReader{
		configs:     make(map[liqov1alpha1.ClusterID]*rest.Config),
		namespaces:  make(map[liqov1alpha1.ClusterID]string),
		secretNames: make(map[liqov1alpha1.ClusterID]string),
	}
}

// Add adds the associations about a remote cluster to the identityReader.
func (i *IdentityReader) Add(clusterID liqov1alpha1.ClusterID, namespace, secretName string, restcfg *rest.Config) *IdentityReader {
...
func (i *IdentityReader) GetConfig(remoteCluster liqov1alpha1.ClusterID, _ string) (*rest.Config, error) {
...
func (i *IdentityReader) GetSecretNamespacedName(remoteCluster liqov1alpha1.ClusterID,
...
func (i *IdentityReader) GetConfigFromSecret(_ liqov1alpha1.ClusterID, _ *corev1.Secret) (*rest.Config, error) {
	panic("implement me")
}

// GetRemoteTenantNamespace retrieves the tenant namespace associated with a remote cluster.
func (i *IdentityReader) GetRemoteTenantNamespace(remoteCluster liqov1alpha1.ClusterID, _ string) (string, error) {
