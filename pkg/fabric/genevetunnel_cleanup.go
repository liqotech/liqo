...
// RunnableGeneveCleanup periodically removes orphan geneve interfaces on the node.
...
func (rgc *RunnableGeneveCleanup) Start(ctx context.Context) error {
	...
	for {
		select {
		case <-ctx.Done():
			// Let's cleanup interfaces immediately when liqo-fabric pod is getting deleted
			if err := geneveCleanup(context.Background(), rgc.Client); err != nil {
				klog.Warningf("geneve cleanup on shutdown failed: %v", err)
			}
			return nil
		case <-ticker.C:
...
func geneveCleanup(ctx context.Context, cl client.Client) error {
	...
	internalfabricsList, err := getters.ListInternalFabricsByLabels(ctx, cl, labels.Everything())
	if err != nil {
		return fmt.Errorf("failed to list internal fabrics: %w", err)
	}
	...
	for _, interfaceItem := range interfaceList {
		...
		if _, ok := internalfabricsMap[name]; !ok {
