...
const liqoctlTestNetworkLongHelp = `Enable an out-of-band peering towards a remote cluster.
...
func newTestCommand(ctx context.Context, f *factory.Factory) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:               "test",
		Short:             "Run tests to check liqo functionalities",
